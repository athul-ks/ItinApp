...
## 2026-07-15 - E2E Mock Bypass in Production

**Vulnerability:** The `x-e2e-mock` HTTP header could bypass the `generate` trip endpoint's credit deduction and rate limiting in production.
**Learning:** Testing backdoors left active in production are a severe security risk, allowing malicious users to bypass business logic and resource constraints.
**Prevention:** Always guard test-specific overrides and mock data logic with a strict `process.env.NODE_ENV !== 'production'` check.
